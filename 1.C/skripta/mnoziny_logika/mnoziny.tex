...
nejpoužívanější) pokus o formalisaci základů, na nichž stojí celá matematika.
Čili, jde o \hyperref[def:teorie]{teorii}, jejímž \hyperref[def:model]{modelem}
je matematika.
...
o pojem \emph{množiny} a symbol náležení $ \in $. Jsou-li $x, y$ dvě množiny,
pak $x \in y$ je \hyperref[def:vyrok]{výrok}, který čteme jako: \uv{$x$ náleží
$y$,} též: \uv{$x$ leží v $y$,} nebo: \uv{$x$ je prvkem $y$,} a znamená, jak
jistě tušíte, že množina $x$ je prvkem množiny $y$. Teorie množin s sebou rovněž
nese několik dalších užitečných symbolů pro množinové operace, které si však
představíme až spolu s \emph{axiomy}, které zaručují jejich existenci.

\section{Axiomy teorie množin}
\label{sec:axiomy-teorie-mnozin}
\input{mnoziny_logika/mnoziny/axiomy}
