...
\begin{frame}
 \frametitle{\texttt{While} cyklus (otevřený cyklus)}
 \begin{tcolorbox}[title=\texttt{While} cyklus,center,width=.9\textwidth]
  Cyklus, který se opakuje, \alert{dokud je splněna nějaká podmínka}.
 \end{tcolorbox}
\end{frame}

\begin{frame}
 \frametitle{Příklad 1: kdo neumí pseudokód}
 \centering
 \begin{minipage}{0.8\textwidth}
  \begin{algorithm}[H]
   \DontPrintSemicolon
   \SetKwData{Stud}{IVTstudent}
   \Input{IVTstudent\hspace{1em}\tcp*[l]{jméno studenta IVT}}
   \Output{$n \in \mathbb{N}$\hspace{3.2em}\tcp*[l]{počet napsaných algoritmů}}
   \BlankLine

   $n\leftarrow 0$\;
   \While{\Stud neumí psát pseudokód}{
    \Stud napíše algoritmus v pseudokódu.\;
    $n\leftarrow n + 1$
   }
   \KwReturn n
  \end{algorithm}
...
\begin{frame}
 \frametitle{Příklad 2: rozklad na prvočísla}
 \centering
 \begin{minipage}{.9\textwidth}
  \scalebox{0.8}{
  \begin{algorithm}[H]
   \DontPrintSemicolon
   \Input{$n \in \mathbb{N}$\hspace{2em}\tcp*[l]{přirozené číslo k rozložení}}
   \Output{$\{(p_i, m_i)\}$\hspace{0.35em}\tcp*[l]{posloupnost dvojic
   (prvočíslo, mocnina)}}
   \BlankLine
   ...
   \While{$n > 1$}{
    ...
    $p_{i+1} \leftarrow $ nejbližší prvočíslo větší než $p_i$\;
    $m_{i+1} \leftarrow 0$\;
    $i \leftarrow i + 1$\;
   }
   \KwReturn $\{(p_i,m_i)\}$
...
\begin{frame}{Příklad 1: věková skupina}
 ...
 \begin{minipage}{.8\textwidth}
  \begin{procedure}[H]
  \DontPrintSemicolon
   \SetKwData{Age}{V}\SetKwData{Day}{d}
   \SetKwData{Month}{m}\SetKwData{Year}{r}
   \SetKwData{CurMonth}{M}\SetKwData{CurYear}{R}
   \SetKwData{CurDay}{D}
   \SetKw{KwProc}{Procedure}

   \nonl\KwProc věk(d, m, r)\;
   \BlankLine

   \Input{datum narození\hspace{2em}\tcp*[l]{den, měsíc, rok}}
   \Output{věk\hspace{11ex + 2.2em}\tcp*[l]{věk člověka (v letech)}}
   \BlankLine
   ...
   \If{$(\CurMonth < \Month)$ \KwOr $(\CurMonth = \Month$ \KwAnd $\CurDay <
   \Day)$}{
    $\Age \leftarrow \Age - 1$\;
   }
   \KwReturn \Age
...
\begin{frame}{Příklad 1: věková skupina (pokr.)}
 ...
 \begin{minipage}{.8\textwidth}
  \begin{algorithm}[H]
   \DontPrintSemicolon
    \SetKwData{Age}{V}\SetKwData{Day}{den}
    \SetKwData{Month}{měsíc}\SetKwData{Year}{rok}
   \Input{datum narození\hspace{2em}\tcp*[l]{den, měsíc, rok}}
   \Output{věková skupina\hspace{2.15em}\tcp*[l]{dítě/dospělý/důchodce}}
   \BlankLine
...
\begin{frame}{Příklad 2: spamming}
 \centering
 \begin{minipage}{.8\textwidth}
  \begin{algorithm}[H]
   \DontPrintSemicolon
   \SetKwData{List}{list}
   \SetKwData{Subj}{subject}
   \SetKwData{Body}{body}
   \SetKwData{Exc}{exc}
   \SetKwData{Name}{name}
   \SetKwData{Email}{e-mail}
   \Input{$\List = \{(\Name, \Email)\}$\hspace{2em}\tcp*[l]{seznam (jmě,
   e-mail)}}
   \Output{nic}
   \BlankLine

   \List $\leftarrow$ seřaď(\List, \Name)\;
   \pause
   \For{$(\Name, \Email) \in \List$}{
    $\Exc \leftarrow \; ! * 1000$\;
    \Subj $\leftarrow$ VYHRÁLI JSTE 300,000 KČ \Exc\;
    \Body $\leftarrow$ Vážený/á/é \Name, ...\;
    pošli(\Email, \Subj, \Body)
   }
  \end{algorithm}
 \end{minipage}
\end{frame}
