...
předchozích) je často obtížné určit. K jejich výpočtu jsou často použity metody
z lineární algebry a obecně metody teorie diskrétních systémů zcela mimo rozsah
tohoto textu.

Přinejmenším v případě limit zadaných \uv{hezkými} vzorci čítajícími podíly
mnohočlenů a odmocnin, je možné obyčejně algebraickými úpravami dojít k
výsledku. Uvedeme si pár stěžejních tvrzení sloužících tomuto účelu.
...
\begin{thmproof}
 ...
 \coloneqq \max(n_a,n_b,n_1)$ a počítejme
 \begin{align*}
